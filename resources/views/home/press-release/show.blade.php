                ...
                @auth
                    <div class="card-header">
                        <a href="{{ route('chambers.press-release.edit', $pressRelease) }}"
                            class="badge badge-pill badge-primary tx-14" target="_blank">Edit Press Release</a>
                    </div>
                @endauth
                    ...
                    <div class="mg-b-10">
                        ...
                        @foreach ($pressRelease->tags as $tag)
                            <a href="{{ route('press-release.index', ['tag' => $tag->slug]) }}"
                                class="badge badge-pill badge-danger tx-14" target="_blank">{{ $tag->name }}</a>
                        @endforeach
                        ...
                        @if ($pressRelease->gunung_api)
                            ...
                            <a href="#"
                                class="badge badge-pill badge-primary tx-14">{{ "Gunung Api {$pressRelease->gunungApi->name}" }}</a>
                        @endif

                        @if ($pressRelease->gerakan_tanah)
                            <a href="{{ route('press-release.index', ['category' => 'gerakan-tanah']) }}"
                                class="badge badge-pill badge-warning tx-14">Gerakan Tanah</a>
                        @endif

                        @if ($pressRelease->gempa_bumi)
                            <a href="{{ route('press-release.index', ['category' => 'gempa-bumi']) }}"
                                class="badge badge-pill badge-warning tx-14">Gempa Bumi</a>
                        @endif

                        @if ($pressRelease->tsunami)
                            <a href="{{ route('press-release.index', ['category' => 'tsunami']) }}"
                                class="badge badge-pill badge-warning tx-14">Tsunami</a>
                        @endif

                        @if ($pressRelease->lainnya)
                            <a href="#" class="badge badge-pill badge-light tx-14">{{ $pressRelease->lainnya }}</a>
                        @endif
